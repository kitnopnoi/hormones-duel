/* Hormone Duel Online - FIXED BUILD */
'use strict';
...
if(!window.supabase) throw new Error('Supabase SDK ไม่ได้ถูกโหลด');
...
const STATUS={
 ...
 STRESS:['เครียด',true],SLEEP:['หลับ',true],DMG_UP:['เพิ่มความเสียหาย',false],
 HEAL_UP:['เพิ่มการฟื้นฟู',false],DMG_REDUCE:['ลดความเสียหายที่ได้รับ',false],
...
['GHRH','GHRH','📈','กระตุ้นต่อมใต้สมองให้หลั่ง GH','ไฮโพทาลามัส','drawGH',0,2],
...
const byName=n=>rawCards.find(c=>c.name===n);
...
function cardOf(x){return rawCards[typeof x==='number'?x:x.cardId]}
function clone(x){return JSON.parse(JSON.stringify(x))}
function buildDeck(){return shuffle(rawCards.flatMap(c=>[c,c])).slice(0,60).map(c=>c.id)}
...
function effectiveDamage(a,t,n,debuff=false){let m=1;if(has(a,'DMG_UP'))m*=1+(a.status.DMG_UP/100);if(has(a,'LOW_CALCIUM')||has(a,'LOW_SUGAR'))m*=.5;if(has(t,'DMG_REDUCE'))m*=1-t.status.DMG_REDUCE/100;if(debuff&&has(t,'DEBUFF_REDUCE'))m*=1-t.status.DEBUFF_REDUCE/100;damage(t,n*m)}
...
function apply(c,a,t){
 ...
 case'thymosin':DEBUFFS.forEach(k=>removeStatus(a,k));addStatus(a,'DEBUFF_REDUCE',c.duration);a.status.DEBUFF_REDUCE=100;break;
...
})[c.effect]||'ใช้ความสามารถของฮอร์โมน'}
...
async function joinRoom(){
 ...
 if(e)return alert('เข้าห้องไม่สำเร็จ: '+e.message);await enter();
}
async function enter(){
 const{data,error}=await supabaseClient.from('rooms').select('*').eq('id',roomId).single();if(error)return alert(error.message);
 room=data;$('roomCodeDisplay').textContent='รหัสห้อง: '+roomId;$('lobby').classList.add('hidden');$('game').classList.remove('hidden');
...
async function play(i){
 ...
 const me=role==='p1'?room.state.p1:room.state.p2,c=cardOf(me.hand[i]);if(!c||!canUse(me,c))return;
 const s=clone(room.state);s.pendingAction={player:role,itemUid:me.hand[i].uid};await save(s);
}
function startHost(){
 if(hostTimer)return;
 hostTimer=setInterval(async()=>{
  if(!room||role!=='p1'||hostBusy)return;hostBusy=true;
  try{const s=room.state,now=Date.now();
...
async function resolve(s){
 const actor=s.pendingAction.player,p=actor==='p1'?s.p1:s.p2,t=actor==='p1'?s.p2:s.p1,i=p.hand.findIndex(x=>x.uid===s.pendingAction.itemUid);
 if(i<0){s.pendingAction=null;return save(s)}
 const c=cardOf(p.hand[i]);if(!canUse(p,c)){p.hand.splice(i,1);s.pendingAction=null;return advance(s)}
 p.hand.splice(i,1);apply(c,p,t);s.lastCard={who:actor==='p1'?'ผู้เล่น 1':'ผู้เล่น 2',emoji:c.emoji,display:c.display,real:c.real,organ:c.organ,ability:ability(c)};
...
function render(){
 ...
 $('pwater').textContent=Math.round(me.water);$('bwater').textContent=Math.round(op.water);$('pwb').style.width=clamp(me.water,0,100)+'%';$('bwb').style.width=clamp(op.water,0,100)+'%';
 $('count').textContent=me.hand.length;$('round').textContent=room.state.round;statuses('pstatus',me);statuses('bstatus',op);
 const n=room.state.timerEndsAt&&room.state.phase==='turn'?Math.max(0,Math.ceil((room.state.timerEndsAt-Date.now())/1000)):0;$('timer').textContent=n?`⏱️ ${n}s`:'⏱️ —';
 ...
 const hand=$('hand');hand.innerHTML='';me.hand.forEach((it,i)=>{const c=cardOf(it),b=document.createElement('button');b.className='card';b.disabled=room.state.phase!=='turn'||room.state.turn!==role||room.state.pendingAction||!canUse(me,c);b.innerHTML=`<div class="cardtop"><span class="emoji">${c.emoji}</span><span class="type">${c.organ}</span></div><h3>${c.display}</h3><div class="real">หน้าที่จริง: ${c.real}</div><div class="organ">สร้างจาก: ${c.organ}</div>`;b.onclick=()=>play(i);hand.appendChild(b)});
 const last=room.state.lastCard;if(last)$('last').innerHTML=`<div><div class="emoji">${last.emoji}</div><h3>${last.who} ใช้ ${last.display}</h3><p><b>ความสามารถ:</b> ${last.ability}</p><p><b>หน้าที่จริง:</b> ${last.real}</p><p><b>สร้างจาก:</b> ${last.organ}</p></div>`;else $('last').textContent='การ์ดที่ใช้จะแสดงตรงนี้';
}
