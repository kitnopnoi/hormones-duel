(() => {
  ...
  const SUPABASE_URL = "https://bqzbjrcghwbytzifnkhf.supabase.co";
  const SUPABASE_ANON_KEY = "sb_publishable_PqqLo641aMtGc0RYuxnfJA_KJ0BwlDv";
  ...
  async function createRoom() {
    ...
    try {
      ...
      console.error(e);
      setMessage("สร้างห้องไม่สำเร็จ: " + describeDbError(e), true);
    } finally {
      state.busy = false;
    }
  }

  // แปลง error จาก Supabase ให้อ่านง่ายขึ้น ช่วยวินิจฉัยปัญหาการตั้งค่าฐานข้อมูล
  function describeDbError(e) {
    const msg = (e && (e.message || e.error_description)) || String(e);
    const code = e && e.code;
    if (code === "42P01" || /relation .* does not exist/i.test(msg)) {
      return 'ยังไม่มีตาราง "rooms" ในฐานข้อมูล กรุณารันสคริปต์ setup-database.sql ในโปรเจกต์ Supabase ของคุณก่อน';
    }
    if (code === "42501" || /permission denied|row-level security/i.test(msg)) {
      return 'สิทธิ์ฐานข้อมูล (RLS Policy) ยังไม่อนุญาตให้สร้าง/อ่านห้อง กรุณารันสคริปต์ setup-database.sql เพื่อเปิดสิทธิ์ที่จำเป็น';
    }
    if (/Failed to fetch|NetworkError/i.test(msg)) {
      return 'เชื่อมต่อ Supabase ไม่ได้ ตรวจสอบอินเทอร์เน็ตหรือ SUPABASE_URL ในไฟล์ game.js';
    }
    return msg;
  }
  ...
  async function joinRoom() {
    ...
    try {
      ...
      console.error(e);
      setMessage("เข้าห้องไม่สำเร็จ: " + describeDbError(e), true);
    } finally {
      state.busy = false;
    }
  }
