(() => {
  ...
  $("myProfile").addEventListener("click", () => openStatusDetails(getMe(), "สถานะของคุณ", true));
  $("enemyProfile").addEventListener("click", () => openStatusDetails(getEnemy(), "สถานะของคู่ต่อสู้", false));
  $("myProfile").addEventListener("keydown", e => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      openStatusDetails(getMe(), "สถานะของคุณ", true);
    }
  });
  $("enemyProfile").addEventListener("keydown", e => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      openStatusDetails(getEnemy(), "สถานะของคู่ต่อสู้", false);
    }
  });
